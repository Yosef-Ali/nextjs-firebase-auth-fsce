...
async function seedPartners() {
  try {
    ...
    for (const partner of partners) {
      ...
      console.log(`✅ Added Strategic Partner: ${partner.name}`);
    }
    
    // Seed Membership Partners
    console.log("\n📍 Seeding Membership Partners...");
    for (const partner of membershipPartners) {
      await addDoc(collection(db, "partners"), {
        ...partner,
        createdAt: new Date(),
        updatedAt: new Date()
      });
      console.log(`✅ Added Membership Partner: ${partner.name}`);
    }
